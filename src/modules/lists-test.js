...
export function initListsTest() {
  ...
  const getPerchanceList = (listName, fallback = []) => {
  ...
  return {
    ...
    testSelectOne(listName = 'itens') {
      ...
      try {
        const list = getPerchanceList(listName, ['item padrão']);
    ...
    testSelectUnique(listName = 'nomes_herois', count = 2) {
      ...
      try {
        const list = getPerchanceList(listName, ['Herói Padrão']);
    ...
    testConsumableList(listName = 'eventos') {
      ...
      try {
        const list = getPerchanceList(listName, ['evento padrão']);
    ...
    testPluralForm(listName = 'itens') {
      ...
      try {
        const list = getPerchanceList(listName, ['item']);
    ...
    testTitleCase(listName = 'adjetivos') {
      ...
      try {
        const list = getPerchanceList(listName, ['adjetivo']);
        const item = typeof list.selectOne === 'function' ? list.selectOne() : list.selectOne;

...
async def install_log_interceptor(cdp, session_ids: list[str]):
    ...
    Captures:
      ...
      - Resource load errors (img, script, link via capture-phase error listener)
      - Fetch failures (intercepted fetch API)
      - XHR failures (intercepted XMLHttpRequest)

    Note: Browser-internal errors (ERR_BLOCKED_BY_CLIENT, cross-origin postMessage)
    are NOT captured by this interceptor. These are generated by the browser engine,
    not by JavaScript, and are typically noise (ad-blockers, security policies).
    """
    interceptor_code = """
        if (!window.__consoleLogs) {
            ...
            // 2. Uncaught errors (scripts)
            window.addEventListener('error', function(e) {
                // e.message already contains 'Uncaught ...' in some browsers, avoid duplication
                var msg = e.message;
            ...
            // 3. Resource load errors (images, scripts, stylesheets — capture phase)
            window.addEventListener('error', function(e) {
...
async def poll_all_frame_logs(cdp, all_sids: list[str], duration: int = 10, interval: float = 0.5) -> list[dict]:
    ...
    Captures (via JS interceptor installed by install_log_interceptor):
      - console.log/warn/error/info/debug
      - Uncaught exceptions
...
[iframe-1] [log] ✅ [Mermaid] Loaded from CDN (v1.6.1)
[iframe-1] [debug] streamKeepAlive sent
[main] [error] Uncaught TypeError: Cannot read properties of null (reading 'style') @ https://perchance.org/cp48hd48rs:7735
[iframe-1] [debug] FINISHED STREAMING.
[iframe-1] [log] ✅ [AI-Text] Texto gerado: Com a espada enferrujada...
[iframe-1] [debug] stopping keepalives due to `finishedGenerating`
```

**Note on browser-internal errors:** The following types of errors appear in DevTools but are **not captured** by this method because they are generated by the browser engine, not by JavaScript:

- `Failed to load resource: net::ERR_BLOCKED_BY_CLIENT` — caused by ad-blockers or browser extensions
- `Failed to execute 'postMessage' on 'DOMWindow'` — cross-origin security errors

These are typically noise and can be safely ignored for functional testing purposes.
...
| Browser-internal errors are not captured | `ERR_BLOCKED_BY_CLIENT`, cross-origin `postMessage` errors are generated by the browser engine, not JS. They are typically noise from ad-blockers and security policies. |
